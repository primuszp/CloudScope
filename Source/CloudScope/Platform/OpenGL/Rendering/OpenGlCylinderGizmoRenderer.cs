...
namespace CloudScope.Platform.OpenGL.Rendering
{
    ...
    /// </summary>
    internal sealed class OpenGlCylinderGizmoRenderer : OpenGlGizmoRendererBase
    {
        ...
        private void RenderWireframe(Matrix4 mvp)
        {
            ...
            GL.Enable(EnableCap.DepthTest);
            DrawLines(_circVbo, 0, totalVerts, new Vector4(0.25f, 0.85f, 0.95f, 0.85f), 1.8f);

            GL.Disable(EnableCap.DepthTest);
            DrawLines(_circVbo, 0, totalVerts, new Vector4(0.25f, 0.85f, 0.95f, 0.18f), 1.0f);
        ...
        private void RenderHeightArrows(CylinderSelectionTool cyl, OrbitCamera cam, Matrix4 vp)
        {
            ...
            for (int gripIdx = 1; gripIdx <= 2; gripIdx++)
            {
                ...
                DrawWorldSpaceArrow(arrow.Start, arrow.Tip, coneLen, coneRad, style.Color, MathF.Max(style.LineWidth, 2.5f), cam, ref vp);
            }
